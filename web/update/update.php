...
if(isset($_POST['update_profile_btn'])) {
	if(!empty($_POST['update_first_name']) && $user_id_web == $id && $token_web == $token) {
		...
		?>
		<div class="flesh-profile flesh-profile-confirm">Данные профиля были изменены<span class="flesh-cross"></span></div>
		<?php		
	}
	else {
		?>
		<div class="flesh-profile flesh-profile-decline">Данные профиля не были изменены<span class="flesh-cross"></span></div>
		<?php
	}
}
...
if(isset($_POST['update_email_btn'])) {
	...
	if(!empty($_POST['update_email']) && filter_var($_POST['update_email'], FILTER_VALIDATE_EMAIL) && $email_unique == 'true' && $password_correct == 'true' && $user_id_web == $id && $token_web == $token) {
		...
		?>
		<div class="flesh-profile flesh-profile-confirm">Email был изменён<span class="flesh-cross"></span></div>
		<?php
	}
	else {
		?>
		<div class="flesh-profile flesh-profile-decline">Email не был изменён<span class="flesh-cross"></span></div>
		<?php
	}	
}
...
if(isset($_POST['update_password_btn'])) {
	...
	if(!empty($_POST['update_old_password']) && !empty($_POST['update_new_password']) && $password_update_correct == 'true' && $user_id_web == $id && $token_web == $token) {
		...
		?>
		<div class="flesh-profile flesh-profile-confirm">Пароль был изменён<span class="flesh-cross"></span></div>
		<?php
	}
	else {
		?>
		<div class="flesh-profile flesh-profile-decline">Пароль не был изменён<span class="flesh-cross"></span></div>
		<?php
	}
}	
				...
				<div class="main-wrap">
					<div class="content update-content">
						...
						<form class="update-form" method="POST" action="<?php echo $_SERVER['PHP_SELF'] ?>">
							...
							<span class="error-message"></span>
							<?php if(!empty($_POST['update_email']) && !filter_var($_POST['update_email'], FILTER_VALIDATE_EMAIL)) echo '<span class="error-message" style="margin-top:5px">Email был введён с ошибкой</span>' ?>
							<?php if(!empty($email) && filter_var($email, FILTER_VALIDATE_EMAIL) && $email_unique == 'false') echo '<span class="error-message" style="margin-top:5px">Этот email уже занят</span>' ?>
