...
while($row_data = mysqli_fetch_array($data)) {
	if($user_id_web == $row_data[0] && $token_web == $row_data[1]) {
		...
		if(isset($_POST['test-result']) && $_POST['test-result'] > $test_1) {
			$test_result = $_POST['test-result'];
			$query_test_result = "UPDATE quiz SET test_1=$test_result WHERE id_users=$user_id_web";
