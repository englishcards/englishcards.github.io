...
form.addEventListener('submit', (event) => {
	...
	let formData = document.querySelector('input[name="test-result3"]').value;
	formData = countResult;
	const request = new XMLHttpRequest();

	request.open('POST', 'quick-test.php', true);
	request.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded; charset=UTF-8');
	request.send('test-result3=' + encodeURIComponent(formData));
	...
	if((countResult) > parseInt(progressResult[2].innerText) && (countResult) >= 80 && parseInt(progressResult[2].innerText) < 80) progressBar.value += 20;
});
